...
var Blessing = require('./lib/blessing');
...
var addonStore = MongoStore(process.env[app.config.MONGO_ENV], 'blessing');
var notifier = Notifier({format: 'html', dir: __dirname + '/messages'});

addon.webhook('room_message', /^\/blessing(?:\s+(:)?(.+?)\s*$)?/i, function *() {
  var global = !this.tenant.room;
  var match = this.match;
  var room = this.room;
  var blessing = Blessing(addonStore, this.tenant);
  var enabled = yield blessing.isEnabled(room.id);
  var command = match && match[1] === ':' && match[2];
  var subject = match && !match[1] && match[2];
  if (command) {
    if (global && command === 'enable') {
      enabled = true;
      yield blessing.setEnabled(room.id, enabled);
      return yield notifier.send('Blessing matching has been enabled in this room.');
    } else if (global && command === 'disable') {
      enabled = false;
      yield blessing.setEnabled(room.id, enabled);
      return yield notifier.send('Blessing matching has been disabled in this room.');
    } else if (/^top\s+things$/.test(command)) {
      return yield notifier.sendTemplate('list', {
        category: 'things',
        type: 'most',
        list: yield blessing.list('thing', true)
      });
    } else if (/^bottom\s+things$/.test(command)) {
      return yield notifier.sendTemplate('list', {
        category: 'things',
        type: 'least',
        list: yield blessing.list('thing', false)
      });
    } else if (/^top\s+users/.test(command)) {
      return yield notifier.sendTemplate('list', {
        category: 'users',
        type: 'most',
        list: yield blessing.list('user', true)
      });
    } else if (/^bottom\s+users/.test(command)) {
      return yield notifier.sendTemplate('list', {
        category: 'users',
        type: 'least',
        list: yield blessing.list('user', false)
      });
    } else {
    ...
    if (subject.charAt(0) === '@') {
      ...
      if (user) {
        subject = user.name;
        value = yield blessing.forUser(user.id);
      } else {
        value = yield blessing.forThing(subject);
      }
    } else {
      value = yield blessing.forThing(subject);
    }
    return yield notifier.send(subject + ' has ' + value + ' blessing.');
  } else {
...
addon.webhook('room_message', new RegExp(strIncDec), function *() {
  ...
  if (!(yield blessing.isEnabled(room.id))) return;
  ...
  while (match = reIncDec.exec(this.content)) {
    ...
    if (isMention) {
      ...
      if (user) {
        if (user.id === sender.id) {
          ...
          value = yield blessing.updateUser(user, change);
        }
      } else {
        subject = match[1];
        if (matches[subject]) continue;
        value = yield blessing.updateThing(subject, change);
      }
    } else {
    ...
    var line = possessive + ' blessing has ' + changed + ' to ' + value;
    if (maxed) {
